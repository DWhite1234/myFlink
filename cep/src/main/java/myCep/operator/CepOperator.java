...
package myCep.operator;

import myCep.EventComparator;
import myCep.configuration.SharedBufferCacheConfig;
import myCep.functions.PatternProcessFunction;
import myCep.functions.TimedOutPartialMatchHandler;
import myCep.nfa.NFA;
import myCep.nfa.NFAState;
import myCep.nfa.NFAStateSerializer;
...
public class CepOperator<IN, KEY, OUT>
    ...
    private void saveRegisterWatermarkTimer() {
        long currentWatermark = timerService.currentProcessingTime();
        System.out.println("currentWatermark:"+currentWatermark);
        // protect against overflow
        if (currentWatermark + 1 > currentWatermark) {
            timerService.registerProcessingTimeTimer(VoidNamespace.INSTANCE, currentWatermark+1);
        }
    }
